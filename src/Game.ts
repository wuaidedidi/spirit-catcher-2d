...
export class Game {
  ...
  private renderEntity(ctx: CanvasRenderingContext2D, entity: Creature): void {
    ...
    if (entity.isAlive) {
      const barW = 24 * this.camera.scale;
  ...
  private renderCaptureAnimation(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.drawImage(
      ballSheet.canvas,
      0, 0, ballSheet.frameWidth, ballSheet.frameHeight,
      Math.floor(screen.x), Math.floor(screen.y),
      ...
      if (dist < 20) {
        if (isBall) {
          this.captureSystem.attemptCapture(
            this.player, creature, selectedItem!.id as BallType,
            this.inventory, this.combatSystem
          );
        } else {
  ...
  private handleScreenClick(sx: number, sy: number, button: number): void {
    ...
    if (this.shopSystem.isOpen) {
      ...
      this.shopUI.handleClick(sx, sy, this.shopSystem, this.player, this.inventory, this.combatSystem, this.screenWidth, this.screenHeight);
